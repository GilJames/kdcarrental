...
<div class="modal fade text-left" id="ModalCreate_Self{{ $bk->id }}{{ $key }}" tabindex="-1" role="dialog"
    aria-hidden="true">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input class="input" type="hidden" name="model" value="selfdrive">
                            </div>
                            ...
                            <div class="rating-container">
                                @for ($t = 1; $t <= 5; $t++)
                                    <input type="checkbox" class="rating-checkbox"
                                        id="car{{ $t }}{{ $key }}{{ $bk->id }}">
                                    <label  onclick="rateCarSelfs({{ $t }}, {{ $key }}, {{ $bk->id }})" for="car{{ $t }}{{ $key }}{{ $bk->id }}"><i
                                            class="fas fa-car"></i></label>
                                @endfor
                            </div>

                            <input type="hidden" name="rating"
                                id="ratingValueSelf{{ $key }}{{ $bk->id }}" value="">
                            @error('rating')
...
<script>
    function rateCarSelfs(rating, key, id) { 
        // Find the modal associated with the clicked button
        var modalSelector = '#ModalCreate_Self' + id + key;
        ...
        $('#ratingValueSelf' + key + id).val(rating);
