...
<div class="modal fade" id="modalself{{ $bk->id }}{{ $key }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true"> 
                ...
                <div class="row">
                    <div class="{{ $bk->lastrate === null ? 'col-md-12' : 'col-md-6' }}">
                        <div>
                            ...
                            <p>Created at: {{ $bk->created_at }} || Updated at: {{ $bk->updated_at }}</p>
                            <br>
                            <br>
                    ...
                    @if ($bk->lastrate != null)
                        <div class="col-md-6">
                            <div class="review_here{{ $bk->id }}">
                                ...
                                <br>
                                <p class="">Comment:</p>
                                <textarea class="form-control" readonly id="comment{{ $bk->id }}{{ $key }}" placeholder="Your Review"
            ...
            <div class="modal-footer">
                <div class="status">
                    @if ($bk->status == 'reject' || $bk->status == 'completed')
                        <a href="{{ url('viewcardetails/' . $bk->admincar->id) }}" class="btn btn-danger reviewBtnatag">Book again</a>
                    @elseif($bk->status == 'confirm')
                        <h2 class="h5 mb-3" style="text-align: center;">
                            <a href="#" class="text-muted">A Confirmation for Your Booking was Sent, Please Check
                                Your Email!</a>
                        </h2>
                ...
                <button type="button" class="btn btn-secondary reviewBtn"
...
<script>
    function checkCarsBasedOnInputValueSelf(modalSelector, id, key, model) {
        $.ajax({
            ...
            success: function(res) {
                ...
                const modalSelector = '#modalself' + id + key;
